...
CSectionStoreBAT::CSectionStoreBAT(UINT16 uwPid, string sName, CTSParser *pTSParser):CSectionObserver(uwPid, sName, pTSParser)
{
}

CSectionStoreBAT::~CSectionStoreBAT(void)
{
}


EResult CSectionStoreBAT::ProcessNewSection(CTSPacketStore * pTSPacketStore)
{
	const UCHAR8 *pucData = pTSPacketStore->GetDataAddr();
	UCHAR8 ucTableId = CSectionHeaderReader::GetTableId(pucData);
	if(TABLEID_BAT == ucTableId)
	{
...
EResult CSectionStoreBAT::GetDescription(CTSPacketStore * pTSPacketStore, CString& strDescription)
{
	const UCHAR8 *pucData = pTSPacketStore->GetDataAddr();
	...
	strDescription.Format(L"TableId 0x%02X, BouquetId 0x%04X, Version 0x%02X, SectionNumber 0x%02X, LastSectionNumber 0x%02X", 
...
EResult CSectionStoreBAT::DisplaySectionData(CTSPacketStore * pTSPacketStore,CTreeList *pTreeList,  CTreeNode * pTreeNodeInput, CTreeNode * pTreeNodeParent)
{
	...
	// 12 Bit bouquet_descriptors_length.
	uiFieldLength = 12;
	uiData = (((pucData[0] &0xF) << 8) | pucData[1]);
	pTreeNode  = new CTreeNode(pTSPacketStore, L"bouquet_descriptors_length", uiOffset, uiFieldLength, uiData );
	pTreeList->InsertAfter( pTreeNode, pTreeNodeTmp);
	pTreeNodeTmp = pTreeNode;
	uiOffset += uiFieldLength;
	...
	else
	{
		...
		// bouquet_descriptors, it is a loop containing descriptors.
		uiFieldLength = uiDescriptorsLoopLength*8;
		pTreeNode  = new CTreeNode(pTSPacketStore, L"bouquet_descriptors loop", uiOffset, uiFieldLength, VALUE_ZERO, NODE_LOOP);
		pTreeList->InsertAfter( pTreeNode, pTreeNodeTmp);
		pTreeNodeTmp = pTreeNode;

		//Parse bouquet_descriptors loop.
